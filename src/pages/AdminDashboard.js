...
const AdminDashboard = () => {
  ...
  const handleDeleteTorneo = async () => {
    try {
      await axios.delete(`${API_URL}/torneos/${torneoToDelete}`);
  ...
  const fetchPosiciones = async (idTorneo) => {
    try {
      const response = await axios.get(
        `${API_URL}/torneos/${idTorneo}/posiciones`
      );
  ...
  const handleAgregarEquipo = async () => {
    ...
    try {
      await axios.post(`${API_URL}/equipos`, nuevoEquipo);
